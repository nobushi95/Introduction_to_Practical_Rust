    ...
    F: Sync + Fn(&T, &T) -> Ordering,
{
    ...
    if x.len().is_power_of_two() {
        ...
        Err(format!(
            "The lenght of x is not a power of two. (x.len(): {})",
            x.len()
        ))
    }
}

const PARARELL_THRESHOLD: usize = 4096;
    ...
    F: Sync + Fn(&T, &T) -> Ordering,
{
    ...
    if x.len() > 1 {
        let mid_point = x.len() / 2;
        let (frist, second) = x.split_at_mut(mid_point);
        if mid_point >= PARARELL_THRESHOLD {
            rayon::join(
                || do_sort(frist, true, comparator),
...
mod tests {
    ...
    fn sort_students_by_name_ascending() {
        ...
        assert_eq!(
            sort_by(&mut x, &|a, b| a
                .last_name
                .cmp(&b.last_name) // まずlast_nameを比較する -> last_nameが等しくなければ（Less or Greater）、それを返す
